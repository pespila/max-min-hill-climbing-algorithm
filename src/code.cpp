...
// [[Rcpp::export]]
NumericVector Svalue(NumericMatrix& A, const NumericVector& cardinality, const NumericVector& pa) {
	...
	if (hDim == 2) {
		...
		for (int i = 0; i < vDim; i++)
		{
			...
			for (int j = 0; j < B.ncol(); j++)
				tmp.push_back(B(j, i));
			// tmp = NumericVector::create(tmp, pa);// B(_, i);
...
public:
	...
	int ReversedHash(NumericVector&, int);
...
int Properties::ReversedHash(NumericVector& array, int size) {
	int out = 0;
	...
	do {
		for (k = 2; k < n; k++) {
			...
			if (OK[hash] == 0) {
				...
				results = Svalue(B, newCard, Iterator);
...
// [[Rcpp::export]]
SEXP MMPC(SEXP x, SEXP a, SEXP N) {
	...
	NumericMatrix AdjMat = BDeu(x, A.cardinality, PC, N);
